...
public class Circle implements Shape {
    ...
    /**
     * Constructor for a circle of radius r located at point p
     * @param p location of circle
    ...
    public boolean checkOverlap(LineSegment target) {
        LineSegment tangent = new LineSegment(new Line(-1 / target.equation.m, this.p), this.p, this.radius);
    ...
    public float getDistance(Point p) {
        return (float)(Math.sqrt(Math.pow(this.p.x - p.x, 2) + Math.pow(this.p.y-p.y, 2))) - this.radius;
    }
     ...
     */
    public Point getLocation() { return this.p; }
